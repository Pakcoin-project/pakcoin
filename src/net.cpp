...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            else if (nInbound >= GetArg("-maxconnections", MAX_CONNECTIONS) - MAX_OUTBOUND_CONNECTIONS)
            {
                closesocket(hSocket);
            }
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            (pnode->nVersion < NOPAKS_VERSION_START || pnode->nVersion >= NOPAKS_VERSION_END)) {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Pakcoin is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    if (semOutbound == NULL) {
        ...
        int nMaxOutbound = min(MAX_OUTBOUND_CONNECTIONS, (int)GetArg("-maxconnections", MAX_CONNECTIONS));
