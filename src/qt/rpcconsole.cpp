...
void RPCConsole::setClientModel(ClientModel *model)
{
    ...
    if(model)
    {
        ...
        connect(ui->peerWidget,SIGNAL(clicked(QModelIndex)),this,SLOT(peer_clicked(QModelIndex)));
...
void RPCConsole::peer_clicked(const QModelIndex &index)
{
    int curRow  = index.row();
    const CNodeCombinedStats *stats = clientModel->getPeerTableModel()->getNodeStats(curRow);
    if (stats)
    {
        updateNodeDetail(stats);
        ui->peerWidget->selectRow(curRow);
        LogPrintf("peer_clicked curRow = %i\n", curRow);
     }
     if (curRow <=1)  {
     		clientModel->getPeerTableModel()->startAutoRefresh();
     } else {
